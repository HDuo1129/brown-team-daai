...
- Chose [Transfermarkt](https://www.transfermarkt.com) over alternatives (Soccerway, Wikipedia) for manager data due to its structured dates, consistent IDs, and coverage depth
- Researched and manually resolved all 65 club name mismatches between the two sources (e.g. Ankaraspor/Osmanlispor sharing one TM entity, the Kayseri/Erciyesspor ambiguity, dual Malatyaspor clubs), producing `team_mapping.csv`
...
**Project structure**
- Separated raw match data onto a dedicated `turkey-data` branch to keep the main branch clean for code and derived features — a deliberate design choice, not a merge target
...
- Match CSVs use a mixed date format (DD/MM/YY for older seasons, DD/MM/YYYY for newer), which requires careful handling when joining manager data.

---

## Next Steps

- Join manager characteristics to match-level data to analyse managerial impact
- Build form/momentum features (rolling results over last N games)
- Explore home advantage by region using `team_location.csv`
